    ...
    <div class="row">
        ...
        <div class="col-lg-3 col-6">
            <div class="small-box bg-warning text-white">
                ...
                <a href="{{ route('pendaftaran.index', ['status' => 'diproses']) }}"
                    class="small-box-footer text-decoration-none text-white">
                    Tindak Lanjut <i class="bi bi-arrow-right-circle"></i>
                </a>
            </div>
        </div>
        ...
        <div class="col-lg-3 col-6">
            <div class="small-box text-bg-success">
                ...
                <a href="{{ route('pendaftaran.index', ['status' => 'diterima']) }}"
                    class="small-box-footer text-decoration-none">
                    Detail <i class="bi bi-arrow-right-circle"></i>
                </a>
            </div>
        </div>
        ...
        <div class="col-lg-3 col-6">
            <div class="small-box text-bg-danger">
                ...
                <svg class="small-box-icon" fill="currentColor" viewBox="0 0 24 24">
                    <path
                        d="M12 10.586l4.95-4.95 1.414 1.414L13.414 12l4.95 4.95-1.414 1.414L12 13.414l-4.95 4.95-1.414-1.414L10.586 12l-4.95-4.95L7.05 5.636 12 10.586z" />
                </svg>
                <a href="{{ route('pendaftaran.index', ['status' => 'ditolak']) }}"
                    class="small-box-footer text-decoration-none">
                    Detail <i class="bi bi-arrow-right-circle"></i>
                </a>
            </div>
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="progress" style="width: 100px; height: 5px;">
                                        <div class="progress-bar" role="progressbar"
                                            style="width: {{ ($sekolah->total_pendaftar / $sekolahs->first()->total_pendaftar) * 100 }}%;"
                                            aria-valuenow="{{ $sekolah->total_pendaftar }}" aria-valuemin="0"
                                            aria-valuemax="{{ $sekolahs->first()->total_pendaftar }}"></div>
                                    </div>
                                </div>
                            </li>
                        @empty
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="progress" style="width: 100px; height: 5px;">
                                        <div class="progress-bar bg-success" role="progressbar"
                                            style="width: {{ ($jurusan->total / $jurusanCounts->first()->total) * 100 }}%;"
                                            aria-valuenow="{{ $jurusan->total }}" aria-valuemin="0"
                                            aria-valuemax="{{ $jurusanCounts->first()->total }}"></div>
                                    </div>
                                </div>
                            </li>
                        @empty
...
@push('script')
    <!-- Chart.js - Hanya muat jika belum ada -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('Data:', chartDataValues);

            // Cek apakah canvas ada
            const canvas = document.getElementById('registrationChart');
            if (!canvas) {
                console.error('Canvas dengan ID registrationChart tidak ditemukan!');
                return;
            }

            // Cek apakah ada data
            if (chartLabels.length === 0 || chartDataValues.length === 0) {
                console.warn('Tidak ada data untuk ditampilkan');
                canvas.parentElement.innerHTML = '<p class="text-center text-muted">Belum ada data pendaftaran</p>';
                return;
            }
